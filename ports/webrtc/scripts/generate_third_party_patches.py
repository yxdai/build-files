...
def GenerateLabeledPaches(patch_dir):
    output_dir = os.path.join(patch_dir, 'webrtc')
    os.makedirs(output_dir)
    data = {'path': '.'}
    with open(os.path.join(output_dir, 'webrtc.meta'), 'w') as f:
        json.dump(data, f)
    ...
    for sha in shas:
        i += 1
        command = ["git", "format-patch", "-k", sha, "-1", "--start-number", str(i), "-o", output_dir]
        result = subprocess.run(
            command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
        )
        if result.returncode != 0:
...
def main(argv=None):
    ...
    for repo in repos:
        if (not repo):
            GenerateLabeledPaches(temp_patch_dir.name)
            continue
        if repo == "qnx_build":
            continue
        repo = repo.strip()
